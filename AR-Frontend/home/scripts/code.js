...
const fetchQuestions = async () => {
    try {
        const res = await axios.get('https://ar-backend-seven.vercel.app/api/v1/question')
...
enterCode.addEventListener('click', () => {
    ...
    if (type == 'coupon') {
        axios.get('https://ar-backend-seven.vercel.app/api/v1/coupon')
            .then(res => {
                ...
                if ((code === '14449' || code == '16901' || code == '11219' || code == '13632' || code == '15958' || code == '17826' || '165123' || '19673' || '16882' || '17395') && coupons.includes(couponCodeMap[code])) {
                    ...
                    axios.delete(`https://ar-backend-seven.vercel.app/api/v1/coupon/${couponCodeMap[code]}`, {
                        headers: {
...
opt1.addEventListener('click', async () => {
    if (opt1.innerText == questionSet.ans) {
        const time = new Date()
        await axios.post('https://ar-backend-seven.vercel.app/api/v1/level', { timestamp: time }, {
            headers: {
                Authorization: localStorage.getItem('uid')
            }
        })
